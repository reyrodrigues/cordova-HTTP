...
- (void)uploadFile:(CDVInvokedUrlCommand*)command {
        ...
        NSData *data = [[NSFileManager defaultManager] contentsAtPath:[filePath stringByReplacingOccurrencesOfString:@"file://"
                                                                       withString:@""]];

        [request setHTTPBody:data];

        CordovaHttpPlugin* __weak weakSelf = self;
        [NSURLConnection sendAsynchronousRequest:request queue:manager.operationQueue completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {

                 NSLog(@"Made it here?");


                 NSMutableDictionary *dictionary = [NSMutableDictionary dictionary];
                 [dictionary setObject:[NSNumber numberWithInt:200] forKey:@"status"];
                 CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:dictionary];
                 [weakSelf.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
         }];
}
